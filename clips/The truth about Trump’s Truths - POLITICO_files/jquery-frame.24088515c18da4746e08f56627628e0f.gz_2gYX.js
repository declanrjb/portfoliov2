        ...
        '_init': function (selector, options) {
            ...
            endLoad = function ($frame, version, data) {
                ...
                if (version >= $frame.data('frame-loadVersion')) {
                    ...
                    body = data;
                    body = body.replace(new RegExp("^[^]*?<body[^>]*>","ig"), '');
                    body = body.replace(new RegExp("<\/body>[/s/S]*?$","ig"), '');
            ...
            loadPage = function ($frame, $source, method, url, data, event) {
                ...
                if (
                    $source.find('input[name="captchaPublicKey"]').val() &&
                    !$source.find('input[name="captchaUserToken"]').val()
                ) {
                    return false;
                }
                ...
                $.ajax({
                    ...
                    'success': function (data, textStatus, jqXHR) {
                        endLoad($frame, version, jqXHR.responseText);
                    },
                    'error': function (jqXHR, textStatus, errorThrown) {
                        console.log('jquery-frame error:', textStatus, errorThrown);
                    }
                });
            };
            ...
            $caller.delegate('a', 'click.frame', function (event) {
                // Ignore privacy links
                if (event.target.closest('.legal-disclaimer')) {
                    return true;
                }
            ...
            if (!$caller.data('click.frame')) {

                // flag item to prevent attaching handler again
                $caller.data('click.frame', true);
            ...
            // check if event handler already exists
            // if exists, skip this item
            if (!$caller.data('submit.frame')) {

                // flag item to prevent attaching handler again
                $caller.data('submit.frame', true);
